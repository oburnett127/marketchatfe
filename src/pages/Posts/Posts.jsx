        ...
        {databasePosts.map((post) => {
          return (
            <Link
              key={post.id}
              to={`/posts/${post._id}`}
              className='bg-blue-900 p-4 rounded-lg border-solid border border-white hover:border-blue-500'
            >
              <div>
        ...
        {posts.map((post) => {
          return (
            <Link
              key={post.id}
              to={`/posts/${post.id}`}
              className='bg-zinc-700 p-4 rounded-lg border-solid border border-white hover:border-blue-500'
            >
              <div>
                <Link to={`/r/stocks`} className='font-bold'>
                  r/stocks
